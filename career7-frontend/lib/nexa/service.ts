...
import type { Career7NexaMessage, Career7NexaRecommendation } from "@/lib/api";
...
function toApiHistory(messages: NexaConversationMessage[]): Career7NexaMessage[] {
...
function toRecommendation(recommendation: Career7NexaRecommendation): NexaRecommendation {
  return {
...
export async function sendNexaChatRequest({
  ...
  try {
    const response = await career7Api.askNexa({
      message,
      quickAction,

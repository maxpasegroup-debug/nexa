...
export default function LoginPage() {
  ...
  async function submit() {
    ...
    try {
      ...
      const response = await fetch("/api/universe/login", {
        method: "POST",
      ...
      if (!response.ok || !data.token) {
        setMessage(data.error ?? "Login failed. Check your phone number and PIN.");
        return;
      }
    ...
    <main className="flex min-h-screen items-center justify-center bg-[radial-gradient(ellipse_at_top,#0d0020_0%,#000000_100%)] px-5 py-10 text-white">
      <section className="w-full max-w-[400px] rounded-[20px] border border-[rgba(245,158,11,0.15)] bg-[#0a0a14] p-8 shadow-2xl">
        <div className="mb-7 text-center">
          <p className="text-sm font-bold uppercase tracking-[0.18em] text-white/35">Returning member</p>
          <h1 className="mt-2 font-heading text-3xl font-extrabold text-[#F59E0B]">Login to 7Universe</h1>
          <p className="mt-2 text-sm text-white/50">Use your phone number and 4-digit PIN.</p>
        </div>
        ...
        <PinBoxes label="Enter your 4-digit PIN" value={pin} onChange={setPin} />
        ...
        <p className="mt-6 text-center text-sm text-white/45">
          New here?{" "}
          <Link href="/register" className="font-bold text-[#F59E0B]">
            Create account
          </Link>
        </p>
      </section>

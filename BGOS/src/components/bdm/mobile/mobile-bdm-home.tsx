...
function Ring({ value, total }: { value: number; total: number }) {
  ...
  const safeTotal = total || 30000;
  const pct = safeTotal > 0 ? Math.min(100, Math.round((safeValue / safeTotal) * 100)) : 0;
  const offset = circumference - (pct / 100) * circumference;
  const stroke = safeValue > 0 ? "#22D9A0" : "rgba(255,255,255,0.18)";
  ...
  return (
    <div className="relative h-[90px] w-[90px]">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
        <span className={`font-heading text-sm font-extrabold ${safeValue > 0 ? "text-[#22D9A0]" : "text-zinc-500"}`}>{money(safeValue)}</span>
        <span className="text-[9px] text-[#6B6878]">of {money(safeTotal)}</span>
      </div>
    </div>
  );
}
...
export function MobileBDMHome({ user, metrics, brief, leads, target, commission }: MobileBDMHomeProps) {
  ...
  const pct = targetAmount > 0 ? Math.min(100, Math.round(((commission.total || 0) / targetAmount) * 100)) : 0;
  const activeLeads = metrics.myLeadsTotal ?? 0;
  const followUpsToday = metrics.followUpsDueToday ?? 0;
    ...
    <main className="mobile-page min-h-screen bg-[#070709] px-4 py-4 text-white">
      ...
      <section className="rounded-2xl border border-white/[0.08] bg-[#13131c] p-4">
        <div className="flex gap-4">
          <Ring value={commission.total || 0} total={targetAmount} />
          <div className="min-w-0 flex-1">
            <span className="inline-flex rounded-full bg-[#7C6FFF]/15 px-3 py-1 text-[11px] font-bold text-[#c6c1ff]">Monthly target</span>
            <p className={`mt-3 font-heading text-[22px] font-extrabold ${commission.total > 0 ? "text-[#22D9A0]" : "text-zinc-500"}`}>{pct}%</p>
      ...
      <section className="mt-4 grid grid-cols-2 gap-3">
        <div className="rounded-[14px] border border-white/[0.08] bg-[#13131c] p-4">
          <p className="text-[11px] text-[#6B6878]">Active leads</p>
          <p className="mt-2 font-heading text-2xl font-extrabold text-[#7C6FFF]">{activeLeads}</p>
        </div>
        <div className="rounded-[14px] border border-white/[0.08] bg-[#13131c] p-4">
          <p className="text-[11px] text-[#6B6878]">Follow-ups today</p>
          <p className="mt-2 font-heading text-2xl font-extrabold text-[#F5A623]">{followUpsToday}</p>
        </div>
      </section>
      ...
      <section className="mt-5">
        ...
        <div className="space-y-3">
          {hotLeads.length > 0 ? hotLeads.map((lead) => <HotLead key={lead.id} lead={lead} />) : (
            <p className="rounded-2xl border border-white/[0.08] bg-[#13131c] p-4 text-xs text-[#6B6878]">
              No hot leads yet. New leads will appear here when they are assigned.
            </p>
          )}
        </div>
      </section>

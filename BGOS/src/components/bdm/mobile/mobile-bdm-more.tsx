        ...
        {rows.map(([icon, label, href]) =>
          href === "#download" ? (
            <button
              ...
              className="flex h-[52px] w-full items-center gap-3 border-b border-white/[0.06] px-4 text-left text-sm"
            >
              <span>{icon}</span>
            ...
            <Link key={label} href={href} className="flex h-[52px] items-center gap-3 border-b border-white/[0.06] px-4 text-sm">
              <span>{icon}</span>
              <span className="flex-1">{label}</span>
              <span>›</span>
            </Link>

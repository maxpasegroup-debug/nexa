    ...
    <div className={`flex ${isUser ? "justify-end" : "justify-start"}`}>
      <div
        className={`max-w-[84%] rounded-2xl px-3 py-2 text-sm leading-5 ${
          isUser
            ? "rounded-tr-sm border border-[rgba(124,111,255,0.3)] bg-[rgba(124,111,255,0.15)] text-white"
  ...
  return (
    <div className="flex justify-start">
      <div className="flex items-center gap-1 rounded-2xl rounded-tl-sm border border-white/10 bg-[#13131c] px-4 py-3">
        <span className="nexa-panel-dot" />
        <span className="nexa-panel-dot delay-150" />
        <span className="nexa-panel-dot delay-300" />
      </div>
    </div>
  );
}
...
export function NexaPanel({
  businessId,
  initialMessage = "brief_morning_update",
}: NexaPanelProps) {
  ...
  async function sendMessage(message: string, visible = true) {
    ...
    if (!response.ok) {
      setMessages((currentMessages) => [
        ...
        {
          role: "nexa",
          content: "I could not fetch the latest data. Try again in a moment.",
        },
      ]);
      return;
    }
  ...
  useEffect(() => {
    ...
    window.addEventListener("nexa:send-message", handleExternalMessage);
    return () =>
      window.removeEventListener("nexa:send-message", handleExternalMessage);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
    ...
    <aside className="flex h-full flex-col bg-[#0e0e13] text-white">
      <header className="flex h-[60px] shrink-0 items-center gap-3 border-b border-white/10 px-4">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            ...
            <span className="rounded-full border border-[#7C6FFF]/30 bg-[#7C6FFF]/10 px-2 py-0.5 text-[10px] font-bold text-[#b8b2ff]">
              CEO
            </span>
          </div>
      ...
      <div className="min-h-0 flex-1 space-y-3 overflow-y-auto px-4 py-4">
        {messages.length > 0 ? (
          ...
          <p className="rounded-2xl border border-white/10 bg-[#13131c] px-4 py-3 text-sm text-zinc-400">
            NEXA is preparing your business briefing.
          </p>
        )}
      ...
      <style jsx>{`
        .nexa-panel-dot {
          ...
          background: #7c6fff;
          animation: nexaPanelPulse 1s ease-in-out infinite;
        }
    ...
    <>
      <div className="fixed bottom-5 right-5 z-40 md:hidden">
        <button
          ...
          className="flex h-14 w-14 items-center justify-center rounded-full bg-[#7C6FFF] text-white shadow-2xl shadow-[#7C6FFF]/30"
